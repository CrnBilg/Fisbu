...
| MOB-027 | AppTypography ölçeği kuruldu (12 adlandırılmış stil), en yüksek etkili "hero" katman (16 konum, 8 dosya) migrate edildi; ~215 konumluk gövde/etiket katmanı MOB-025 olarak backlog'da kaldı | Orta — renk/görsel profesyonellik denetimi, madde 3 | L | Done (kısmi kapsam, şeffaf raporlandı) | - |
| MOB-028 | Ertelenen estetik öneriler 5-8: global sayfa geçiş animasyonu (PageTransitionsTheme), fiş kartı→detay Hero geçişi, başarı mikro-animasyonu (SuccessCheckOverlay), 9 RefreshIndicator'a marka rengi | Düşük — ertelenen estetik öneriler | M | Done | - |
| MOB-029 | Grup 1 işlevsel denetimi madde 1: dashboard "Son Fişlerin" listesi artık tıklanabilir (receipt_list'teki _goToDetail deseniyle tutarlı) | Yüksek — Grup 1 işlevsel denetim | S | Done | - |
| ARCH-008 | Grup 1 işlevsel denetimi madde 2: fiş düzenleme özelliği — backend'e PUT /receipts/{id} (UpdateReceiptUseCase + testler) eklendi, mobilde AddReceiptScreen düzenle modunda yeniden kullanıldı, gerçek emulator'da uçtan uca kanıtlandı | Yüksek — Grup 1 işlevsel denetim | M-L | Done | - |
