...
- **Backend bağımlılığı**: gerçek bir backend'e ihtiyaç duyar. Production Supabase'e KESİNLİKLE bağlanılmaz — yerel/ephemeral bir backend (Testcontainers/docker Postgres + `./gradlew bootRun`, env var override'larıyla) kullanılır. `ApiClient.baseUrl`, `--dart-define=API_BASE_URL=...` ile override edilebilir (varsayılan davranış — production URL — değişmez).
